...
end product to be a sorted aligned BAM or CRAM file.
...
* Mark duplicates (part 1)
* Sort to positional order
* Mark duplicates (part 2)
...
This output file will be in as-it-comes order, either the original
input file ordering or some approximation of that depending on how the
multi-threading works, however the read pairs will be collated next to
each other.

This is important as the next step requires name-collated data.
...
step.  It ensures the SAM FLAG, RNEXT, PNEXT and TLEN are correct and
internally consistent.

Note `samtools fixmate`, as with other sub-commands, can read SAM
directly without needing an explicit option or use of the `view`
command first.
...
#### Marking duplicates (part 1)

Marking duplicates works best with some analysis done with data in
read-name collated order, and some analysis done in genome-position
...
order with:
    ...
    samtools sort -l 1 -@8 -o possrt.bam -T /tmp/example_prefix fixmate.bam

Sort is highly parallel so the `-@8` option here enables to use of 4
additional CPU threads.  It can be sped up by providing it with more
memory, but note the memory option (`-m`) is per-thread.

#### Marking duplicates (part 2)

The main core of marking duplicates may now be ran on the
position-sorted file.

    samtools markdup -O bam,level=1 possrt.bam markdup.bam
...
compression level and output format may be specified in that command.
See below for an example.

#### Joining it all together

You may notice that the output from each stage is used solely as the
input to the next stage.  While the above may allow you to specify the
commands in a workflow language (e.g. CWL) they are not the most
efficient method.
...
one very simple way of reducing long-term storage requirements.
    ...
    samtools sort -n -@4 final.cram | \
    samtools fastq - -1 dat_1.fq -2 dat_2.fq -o /dev/null -s /dev/null
...
However note that most sequence assemblers just produce a consensus,
in FASTA or FASTQ format.  If you wish to get the BAM or CRAM file of
data aligned against this consensus, for purposes of curation or
downstream analysis, then simply follow the Mapping section above
(with an additional step, if required, of building an index on the
assembly consensus prior to mapping).
